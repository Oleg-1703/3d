#!/usr/bin/env python3
"""
Полная версия scene/gaussian_model.py с BasicPointCloud и GaussianModel
Замените содержимое scene/gaussian_model.py этим кодом
"""
...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int):
        ...
        self.num_objects = 256  # Default number of object classes
    ...
    def get_objects(self):
        if hasattr(self, '_objects_dc') and self._objects_dc.numel() > 0:
            return self._objects_dc.squeeze(-1).int()  # (N, 1)
        else:
            # Return default object IDs if not initialized
            return torch.ones(self.get_xyz.shape[0], 1, dtype=torch.int32, device=self.get_xyz.device)
    ...
    def create_from_pcd(self, pcd, spatial_lr_scale: float):
        ...
        # Initialize object IDs randomly
        if hasattr(self, 'num_objects'):
            ...
            fused_object_ids = torch.ones((fused_point_cloud.shape[0], 1, 1), device=device)
    ...
    def update_learning_rate(self, iteration):
        ''' Learning parameter scheduling per step '''
    ...
    def save_ply_with_object_id(self, path):
        """Save PLY file with object IDs as additional attribute"""
        mkdir_p(os.path.dirname(path))
        ...
        # Add object IDs
        if hasattr(self, '_objects_dc') and self._objects_dc.numel() > 0:
            object_ids = self._objects_dc.detach().cpu().numpy().squeeze()
        else:
            object_ids = np.ones(xyz.shape[0])

        # Construct attribute list including object_id
        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes()]
        dtype_full.append(('object_id', 'i4'))  # Integer for object ID

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, object_ids.reshape(-1, 1)), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
    ...
    def load_ply(self, path):
        ...
        # Load object IDs if available
        object_ids = None
        if 'object_id' in [p.name for p in plydata.elements[0].properties]:
            object_ids = np.asarray(plydata.elements[0]["object_id"])[..., np.newaxis, np.newaxis]
        ...
        if object_ids is not None:
            self._objects_dc = nn.Parameter(torch.tensor(object_ids, dtype=torch.float, device=device).requires_grad_(False))
        else:
            # Initialize with default object IDs if not present in PLY
            self._objects_dc = nn.Parameter(torch.ones((xyz.shape[0], 1, 1), dtype=torch.float, device=device).requires_grad_(False))
    ...
    def prune_points(self, mask):
        ...
        # Prune object IDs
        if hasattr(self, '_objects_dc') and self._objects_dc.numel() > 0:
            self._objects_dc = nn.Parameter(self._objects_dc[valid_points_mask].requires_grad_(False))
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_objects_dc=None):
        ...
        # Handle object IDs
        if new_objects_dc is not None:
            if hasattr(self, '_objects_dc') and self._objects_dc.numel() > 0:
                self._objects_dc = nn.Parameter(torch.cat([self._objects_dc, new_objects_dc], dim=0).requires_grad_(False))
            else:
                self._objects_dc = nn.Parameter(new_objects_dc.requires_grad_(False))
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # Handle object IDs for split points
        new_objects_dc = None
        if hasattr(self, '_objects_dc') and self._objects_dc.numel() > 0:
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        # Handle object IDs for cloned points
        new_objects_dc = None
        if hasattr(self, '_objects_dc') and self._objects_dc.numel() > 0:
